window.onload = () => {
    ...
    let [firstIphone, secondIphone, thirdIphone] = document.querySelectorAll('#first-iphone, #second-iphone, #third-iphone');
    ...
    let all = document.querySelectorAll('.inner-camera-roll, .inner-camera, .square-container, .iphone-inner-photos, .video-container, .inner-weather, .inner-watch, .inner-clock, .inner-maps, .calc, .inner-reminders, .inner-iBooks, .inner-find-friends, .inner-find-iphone, .inner-telegram, .inner-telegram-settings');
    for(let one of all) {
    ...
    let [leftToggle, rightToggle] = document.querySelectorAll('.left-toggle, .right-toggle');
  
    let sixthPanelIcons = document.querySelectorAll('.tel, .safari, .message, .music');
    ...
    let [simStatusInnerExtras, batteryStatusInnerExtras, batteryStatus2InnerExtras] = document.querySelectorAll('.sim-status-inner-extras, .battery-status-inner-extras, .battery-status-2-inner-extras');
  
    let timeInnerExtras = document.querySelectorAll('.time-inner-extras');
  
    let iphone = document.querySelector('.iphone');
  
    let [photosIcon, photos, cameraRollAlbum, cameraRoll] = document.querySelectorAll('.photos, .iphone-inner-photos, .camera-roll-album, .inner-camera-roll');
  
    let [favouritesAlbum, favourites] = document.querySelectorAll('.favourites-album, .inner-favourites');
  
    favourites.style.display = 'none';
  
    let [calendarIcon, innerCalendar] = document.querySelectorAll('.calendar, .inner-calendar');
  
    let backButtonInnerCalendarTopContainer = document.querySelector('#back-button-inner-calendar-top-container');
  
    backButtonInnerCalendarTopContainer.innerHTML += new Date().getFullYear();
  
    let imageHeartIcon = document.querySelector('#heart-icon');
  
    let [backToAlbumsButton, backToAlbumsButtonInnerFavourites, innerFavouritesImage] = document.querySelectorAll('#back-to-albums-button, #back-to-albums-button-inner-favourites, .inner-favourites-image');
  
    let backArrowInnerImage = document.querySelector('#back-arrow-inner-image');
  
    let backArrowInnerFavouriteImage = document.querySelector('#back-arrow-inner-favourites-image');
    ...
    let openedImageInnerFavourites = document.querySelector('.opened-image-inner-favourites');
  
    let innerImageBottomContainer = document.querySelector('.inner-image-bottom-container');
  
    innerImageBottomContainer.style.display = 'none';
  
    openedImage.style.display = 'none';
  
    let [innerFavouritesImageTopContainer, innerFavouritesImageBottomContainer] = document.querySelectorAll('.inner-favourites-image-top-container, .inner-favourites-image-bottom-container');
  
    innerFavouritesImageTopContainer.style.display = innerFavouritesImageBottomContainer.style.display = 'none';
  
    let images = document.querySelectorAll('.image-1,.image-2,.image-3,.image-4,.image-5,.image-6,.image-7,.image-8,.image-9,.image-10,.image-11,.image-12');
  
    let worldClockTextInnerClock = document.querySelector('.world-clock-text-inner-clock');
  
    let passcodeInput = document.querySelector('#passcode-input');
    let passCode = `${Math.floor( Math.random() * 10 )}${Math.floor( Math.random() * 10 )}${Math.floor( Math.random() * 10 )}${Math.floor( Math.random() * 10 )}${Math.floor( Math.random() * 10 )}${Math.floor( Math.random() * 10 )}`;
  
    let doneButtonInnerPasscodeContainer = document.querySelector('#done-button-inner-passcode-container');
...
// //   Always your's: "★ Don’t Know ★".`);
    ...
    doneButtonInnerPasscodeContainer.onclick = () => {
      if (passcodeInput.value == passCode) {
        ...
        startContainer.classList.remove('run-start-container-transform-animation');
        setTimeout(() => {
    ...
    let batteryStatus = document.querySelectorAll('#battery-status, #battery-status-2');
  
    let [clockIcon, innerClock] = document.querySelectorAll('.clock, .inner-clock');
  
    let circleWrapper = document.querySelector('.circle-wrapper');
    let circleUnder = document.querySelector('.circle-under');
  
    let weatherUnderMapsDestinations = document.querySelector('.weather-under-maps-destinations');
  
    let [circle1, circle2, circle3, circle4, circle5, circle6] = document.querySelectorAll('.circle-1, .circle-2, .circle-3, .circle-4, .circle-5, .circle-6');
  
    let [firstToggle, secondToggle, thirdToggle, fourthToggle, fifthToggle, sixthToggle] = document.querySelectorAll('.first-toggle, .second-toggle, .third-toggle, .fourth-toggle, .fifth-toggle, .sixth-toggle');
  
    let [firstOff, secondOff, thirdOff, fourthOff, fifthOff, sixthOff] = document.querySelectorAll('.first-off, .second-off, .third-off, .fourth-off, .fifth-off, .sixth-off');
  
    let [firstOn, secondOn, thirdOn, fourthOn, fifthOn, sixthOn] = document.querySelectorAll('.first-on, .second-on, .third-on, .fourth-on, .fifth-on, .sixth-on');
  
    let [firstAlarmTime, secondAlarmTime, thirdAlarmTime, fourthAlarmTime, fifthAlarmTime, sixthAlarmTime] = document.querySelectorAll('.first-alarm-time, .second-alarm-time, .third-alarm-time, .fourth-alarm-time, .fifth-alarm-time, .sixth-alarm-time');
  
    let [firstAlarmText, secondAlarmText, thirdAlarmText, fourthAlarmText, fifthAlarmText, sixthAlarmText] = document.querySelectorAll('.first-alarm-text, .second-alarm-text, .third-alarm-text, .fourth-alarm-text, .fifth-alarm-text, .sixth-alarm-text');
  
    let textWorldClockInnerBottomContainer = document.querySelector('#text-world-clock-inner-bottom-container');
    let alarmTextInnerBottomContainer = document.querySelector('#alarm-text-inner-bottom-container');
    let bedtimeTextInnerBottomContainer = document.querySelector('#bedtime-text-inner-bottom-container');
    let stopwatchTextInnerBottomContainer = document.querySelector('#stopwatch-text-inner-bottom-container');
    let timerTextInnerBottomContainer = document.querySelector('#timer-text-inner-bottom-container');
    ...
    deleteButton.onclick = () => {
      let otp = telNumberOutput.innerHTML.slice(0, telNumberOutput.innerHTML.length - 1);
    ...
    let topWrapperInnerBottomContainerInnerMaps = document.querySelector('.top-wrapper-inner-bottom-container-inner-maps');
    ...
    topWrapperInnerBottomContainerInnerMaps.onclick = () => {
      ...
      if (containerStyles.transform == "matrix(1, 0, 0, 1, 0, -230)") {
        bottomContainerInnerMaps.classList.add('run-maps-bottom-container-animation-2');
        setTimeout(() => {
          bottomContainerInnerMaps.classList.remove('run-maps-bottom-container-animation-2');
          bottomContainerInnerMaps.style.transform = 'translateY(0)';
        }, 400);
      } else {
        bottomContainerInnerMaps.classList.add('run-maps-bottom-container-animation');
        setTimeout(() => {
          bottomContainerInnerMaps.classList.remove('run-maps-bottom-container-animation');
          bottomContainerInnerMaps.style.transform = 'translateY(-230px)';
        }, 400);
      }
    }
  
    let [keynoteIcon, innerKeynote] = document.querySelectorAll('.keynote, .inner-keynote');
    let [garagebandIcon, innerGarageBand] = document.querySelectorAll('.garageband, .inner-garageband');
  
    let [safariIcon, innerSafari] = document.querySelectorAll('.safari, .inner-safari');
    let [mapsIcon, innerMaps] = document.querySelectorAll('.map, .inner-maps');
    let [numbersIcon, innerNumbers] = document.querySelectorAll('.numbers-icon, .inner-numbers');
  
    let bedtimeInnerBottomContainer = document.querySelector('.bedtime-inner-bottom-container');
    let innerClockBedtimeContainer = document.querySelector('.inner-clock-bedtime-container');
  
    let [bedtimeTextInnerMiddleContainer, timeInnerMiddleContainer] = document.querySelectorAll('#bedtime-text-inner-middle-container, #time-inner-middle-container');
    let [wakeTextInnerMiddleContainer, wakeTimeInnerMiddleContainer] = document.querySelectorAll('#wake-text-inner-middle-container, #wake-time-inner-middle-container');
  
    let innerClockTimerContainer = document.querySelector('.inner-clock-timer-container');
    let timerInnerBottomContainer = document.querySelector('.timer-inner-bottom-container');
  
    let [remindersIcon, innerReminders] = document.querySelectorAll('.reminders, .inner-reminders');
    ...
    let titleInnerTopContainer = document.querySelector('#title-inner-top-container');
  
    let [innerPhoneFavourites, innerPhoneRecents, innerPhoneContacts, innerPhoneKeypad, innerPhoneVoicemail] = document.querySelectorAll('.inner-phone-favourites, .inner-phone-recents, .inner-phone-contacts, .inner-phone-keypad, .inner-phone-voicemail');
  
    let favouritesIconContainerInnerPhone = document.querySelector('.favourites-icon-container-inner-phone');
    let recentsIconContainerInnerPhone = document.querySelector('.recents-icon-container-inner-phone');
    let contactsIconContainerInnerPhone = document.querySelector('.contacts-icon-inner-phone');
    let keypadIconContainerInnerPhone = document.querySelector('.keypad-icon-container');
    let voicemailIconContainerInnerPhone = document.querySelector('.voicemail-icon-container');
  
    let favouritesTextInnerPhoneBottomContainer = document.querySelector('#favourites-text-inner-phone-bottom-container');
    let recentsTextInnerPhoneBottomContainer = document.querySelector('#recents-text-inner-phone');
    let contactsTextInnerPhoneBottomContainer = document.querySelector('#contacts-text-inner-bottom-container');
    let keypadTextInnerPhoneBottomContainer = document.querySelector('#keypad-text');
    let voicemailTextInnerPhoneBottomContainer = document.querySelector('#voicemail-text');
  
    let recentsContainerInnerNumbers = document.querySelector('.recents-container-inner-numbers');
    let browseContainerInnerNumbers = document.querySelector('.browse-container-inner-numbers');
    let innerNumbersBrowseContainer = document.querySelector('.inner-numbers-browse-container');
    let recentsTextInnerNumbers = document.querySelector('#recents-text-inner-numbers');
    let browseTextInnerNumbers = document.querySelector('#browse-text-inner-numbers');
    ...
    let doneButtonInnerReminders = document.querySelector('#done-button-inner-reminders');
  
    let timeInnerStartContainer = document.querySelector('#time-inner-start-container');
  
    let [facetimeIcon, innerFacetime] = document.querySelectorAll('.facetime, .inner-facetime');
    let [iMovieIcon, innerIMovie] = document.querySelectorAll('.iMovie, .inner-iMovie');
    let [pagesIcon, innerPages] = document.querySelectorAll('.pages, .inner-pages');
    let innerPagesBrowseContainer = document.querySelector('.inner-pages-browse-container');
    let browseContainerInnerPages = document.querySelector('.browse-container-inner-pages');
    let browseTextInnerPages = document.querySelector('#browse-text-inner-pages');
    let recentsContainerInnerPages = document.querySelector('.recents-container-inner-pages');
    let recentsTextInnerPages = document.querySelector('#recents-text-inner-pages');
  
    let [phoneIcon, innerPhone] = document.querySelectorAll('.tel, .inner-phone');
  
    let bottomRemindersContainer = document.querySelector('.inner-reminders-bottom-reminders-container');
    ...
    let [messageIcon, innerMessage] = document.querySelectorAll('.message, .inner-messages');
  
    let [musicIcon, innerMusic] = document.querySelectorAll('.music, .inner-music');
  
    let [healthIcon, innerHealth] = document.querySelectorAll('.health, .inner-health');
  
    let [videoIcon, innerVideo] = document.querySelectorAll('.video, .inner-video');
  
    let [iTunesStoreIcon, innerItunesStore] = document.querySelectorAll('.iTunes-store, .inner-itunes-store');
  
    let startContainer = document.querySelector('.start-container');
  
    let storeButton = document.querySelector('#store-button');
  
    let [appStoreIcon, innerAppStore] = document.querySelectorAll('.app-store, .inner-app-store');
  
    let [stocksIcon, innerStocks] = document.querySelectorAll('.stocks, .inner-stocks');
  
    let mainWrapperInnerStocks = document.querySelector('.main-wrapper-inner-stocks');
  
    let innerStocksBottomContainer = document.querySelector('.inner-stocks-bottom-container');
  
    let deleteContainerInnerNewNote = document.querySelector('.delete-container-inner-new-note');
  
    let sololearnIcon = document.querySelector('.sololearn');
  
    let [learnIconContainer, playIconContainer, feedIconContainer, curlyBracketIconContainer, discussIconContainer] = document.querySelectorAll('.learn-icon-container, .play-icon-container, .feed-icon-container, .curly-bracket-icon-container, .discuss-icon-container');
    ...
    let [innerBedtimeOff, innerBedtimeOn] = document.querySelectorAll('.inner-bedtime-off, .inner-bedtime-on');
  
    let [innerClockTopContainer, alarmInnerBottomContainer] = document.querySelectorAll('.inner-clock-top-container, .alarm-inner-bottom-container');
  
    let circleInnerToggle = document.querySelector('.circle-inner-toggle');
  
    let innerClockAlarmContainer = document.querySelector('.inner-clock-alarm-container');
  
    let worldClockInnerBottomContainer = document.querySelector('.world-clock-inner-bottom-container');
  
    let [editTextInnerClock, plusButtonInnerClock] = document.querySelectorAll('.edit-text-inner-clock, .plus-button-inner-clock');
  
    let toggleInnerBedtimeContainer = document.querySelector('.toggle-inner-bedtime-container');
  
    let circleInnerBedtimeContainer = document.querySelector('.circle-inner-bedtime-container');
  
    let innerClockStopwatchContainer = document.querySelector('.inner-clock-stopwatch-container');
  
    let stopwatchInnerBottomContainer = document.querySelector('.stopwatch-inner-bottom-container');
  
    let [watchIcon, innerWatch] = document.querySelectorAll('.watch, .inner-watch');
  
    innerClockAlarmContainer.style.display = 'none';
  
    let innerImageTopContainer = document.querySelector('.inner-image-top-container');
    innerImageTopContainer.style.display = 'none';
  
    let offContainer = document.querySelector('.off-container')
  
    let [cameraRollTextInnerCameraRoll, selectTextInnerCameraRoll] = document.querySelectorAll('#camera-roll-top-container-text, #select-text');
  
    let [divideButton, multButton, minusButton, plusButton] = document.querySelectorAll('.divide, .mult, .minus, .plus');
  
    let [iPhoneCalculatorIcon,iPhoneCalculator] = document.querySelectorAll('.calculator, .calc');
  
    let [currentTimeInnerClock, timeOfTheSecondCity, timeOfTheThirdCity, timeOfTheFourthCity, timeOfTheFifthCity, timeOfThesixthCity] = document.querySelectorAll('.current-time-inner-clock, .time-of-the-second-city, .time-of-the-third-city, .time-of-the-fourth-city, .time-of-the-fifth-city, .time-of-the-sixth-city');
  
    let cleanerButton = document.querySelector('.cleaner');
  
    let dateInnerStartContainer = document.querySelector('#date-inner-start-container');
  
    let calcInput = document.querySelector('.calculatorInput');
  
    let homeButton = document.querySelector('.iphone-home');
  
    let [readingNowContainer, libraryContainerInnerIBooks, searchContainerInnerIBooks] = document.querySelectorAll('.reading-now-container, .library-container-inner-iBooks, .search-container-inner-iBooks');
  
    let [readingNowTextInnerIBooks,libraryTextInnerIBooks,searchTextInnerIBooks] = document.querySelectorAll('#reading-now-text-inner-iBooks, #library-text-inner-iBooks, #search-text-inner-iBooks');
  
    let innerIBooks = document.querySelector('.inner-iBooks');
    let innerIBooksLibraryContainer = document.querySelector('.inner-iBooks-library-container');
    let innerIBooksSearchContainer = document.querySelector('.inner-iBooks-search-container');
    ...
    let innerCameraBottomContainerTopWrapper = document.querySelector('.inner-camera-bottom-container-top-wrapper');
    ...
    let innerExtrasBlurContainer = document.querySelector('.inner-extras-blur-container');
  
    let editButton = document.querySelector('#edit-button');
  
    let iBooksIcon = document.querySelector('.iBooks');
  
    let textareaInnerScheduled = document.querySelector('#textarea-inner-scheduled');
  
    let plusInnerScheduled = document.querySelector('#plus-inner-scheduled');
  
    let scheduledMiddleContainer = document.querySelector('.scheduled-middle-container');
  
    let itemsCondition = document.querySelector('#items-condition');
  
    let textareaInnerBottomWrapper = document.querySelector('#textarea-inner-bottom-wrapper');
    ...
    let innerVideoContainerPhoto = document.querySelector('.inner-video-container-photo');
  
    let weatherIcon = document.querySelector('.weather');
    let innerWeather = document.querySelector('.inner-weather');
  
    let [tipsIcon, voiceIcon, contactsIcon, findFriendsIcon, findIphoneIcon] = document.querySelectorAll('.tips-icon, .voice-icon, .contacts-icon, .find-friends-icon, .find-iphone-icon');
    ...
    let callsToggleInnerTelegram = document.querySelector('#calls-toggle-inner-telegram');
    let recentCallsButtonInnerTelegram = document.querySelector('#recent-calls-button-inner-telegram');
    let missedCallsButtonInnerTelegram = document.querySelector('#missed-calls-button-inner-telegram');
    let callsTextInnerMiddleContainer = document.querySelector('#calls-text-inner-middle-container');
    ...
    missedCallsButtonInnerTelegram.onclick = () => {
      ...
      callsToggleInnerTelegram.classList.add('run-toggle-telegram-calls-animation');
      setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        callsToggleInnerTelegram.classList.remove('run-toggle-telegram-calls-animation');
      }, 200);
    }
    ...
    recentCallsButtonInnerTelegram.onclick = () => {
      ...
      callsToggleInnerTelegram.classList.add('run-toggle-telegram-calls-animation-2');
      setTimeout(() => {
        recentCallsButtonInnerTelegram.style.color = '#000';
        missedCallsButtonInnerTelegram.style.color = '#525252';
      }, 100);
  
      callsTextInnerMiddleContainer.innerHTML = 'Your recent calls will appear here';
  
      setTimeout(() => {
        callsToggleInnerTelegram.style.left = '2px';
        callsToggleInnerTelegram.classList.remove('run-toggle-telegram-calls-animation-2');
      }, 200);
    }
  
    let [innerTelegram, innerCallsContainerInnerTelegram, innerChatsContainerInnerTelegram, innerTelegramSettings] = document.querySelectorAll('.inner-telegram, .inner-calls-container-inner-telegram, .inner-chats-container-inner-telegram, .inner-telegram-settings');
    let [innerTelegramContactsIconContainer, innerTelegramCallsContainer, innerTelegramChatsIconContainer, innerTelegramSettingsIconContainer] = document.querySelectorAll('.inner-telegram-contacts-icon-container, .inner-telegram-calls-container, .inner-telegram-chats-icon-container, .inner-telegram-settings-icon-container');
    let [contactsTextInnerTelegramBottomContainer, callsTextInnerTelegramBottomContainer, chatsTextInnerTelegramBottomContainer, settingsTextInnerTelegramBottomContainer] = document.querySelectorAll('#contacts-text-inner-telegram-bottom-container, #calls-text-inner-telegram-bottom-container, #chats-text-inner-telegram-bottom-container, #settings-text-inner-telegram-bottom-container');
  
    let [innerSololearn, playContainerInnerSololearn, activityFeedContainerInnerSololearn, codePlaygroundContainer, discussContainer] = document.querySelectorAll('.inner-sololearn, .play-container-inner-sololearn, .activity-feed-container-inner-sololearn, .code-playground-container, .discuss-container');
  
    let [learnTextInnerBottomContainer, playTextInnerBottomContainer, feedTextInnerBottomContainer, codeTextInnerBottomContainer, discussTextInnerBottomContainer] = document.querySelectorAll('#learn-text-inner-bottom-container, #play-text-inner-bottom-container, #feed-text-inner-bottom-container, #code-text-inner-bottom-container, #discuss-text-inner-bottom-container');
    ...
    doneButtonInnerReminders.onclick = () => {
      ...
      textareaInnerScheduled.insertAdjacentHTML('beforebegin', `<div class='delete-reminder-${countOfReminders} delete-reminder'></div> <p class='reminder-${countOfReminders} reminder'>${textareaInnerScheduled.value}</p>`);
      textareaInnerBottomWrapper.insertAdjacentHTML('beforebegin', `<div class='delete-reminder-inner-bottom-container-${countOfReminders} delete-reminder-inner-bottom-container'></div> <p class='reminders-inner-bottom-container-${countOfReminders} reminder-inner-bottom-container'>${textareaInnerScheduled.value}</p>`);
  
      let deleteRemindersButtons = document.querySelectorAll(`.delete-reminder-${countOfReminders}`);
      let deleteRemindersButtonsInnerBottomContainer = document.querySelectorAll(`.delete-reminder-inner-bottom-container-${countOfReminders}`);
      ...
      for(let deleteRemindersButtonInnerBottomContainer of deleteRemindersButtonsInnerBottomContainer) {
        deleteRemindersButtonInnerBottomContainer.onclick = (e) => {
          ...
          e.target.nextElementSibling.classList.add('run-delete-reminder-animation');
          e.target.classList.add('run-delete-reminder-animation');
  
          setTimeout(() => {
            plusInnerReminders.style.top = `${plusInnerRemindersPos}px`;
            e.target.nextElementSibling.classList.remove('run-delete-reminder-animation');
    ...
    circleInnerBedtimeContainer.onclick = () => {
      if (toggleInnerBedtimeContainer.style.background == 'rgb(102, 231, 58)') {
        ...
        setTimeout(() => {
          ...
          bedtimeTextInnerMiddleContainer.style.opacity = timeInnerMiddleContainer.style.opacity = wakeTextInnerMiddleContainer.style.opacity = wakeTimeInnerMiddleContainer.style.opacity = '1';
    ...
    window.insertAndOperations = function(str) {
        ...
        if (calcInput.innerHTML.startsWith('0') && !calcInput.innerHTML.includes(',')) {
          calcInput.innerHTML = calcInput.innerHTML.slice(0,-1);
        }
        calcInput.innerHTML += str;
  
        if (calcInput.innerHTML.startsWith(',') && calcInput.innerHTML.length == 1) {
          calcInput.innerHTML = '0' + calcInput.innerHTML;
        }
  
        if (calcInput.innerHTML.startsWith('-,') && calcInput.innerHTML.length == 2) {
          calcInput.innerHTML = calcInput.innerHTML.replace('-,', '-0,')
        }
  
        if (calcInput.innerHTML.startsWith('-00') && !calcInput.innerHTML.includes(',')) {
          calcInput.innerHTML = calcInput.innerHTML.replace('0','');
        }
  
        window.mult = function() {
          if (divideButton.style.background == 'white' || minusButton.style.background == 'white' || plusButton.style.background == 'white') {
            return;
          }
          multButton.style.background = multButton.style.background == 'white' ? 'orange' : 'white';
          multButton.style.color = multButton.style.color == 'orange' ? 'white' : 'orange';
          window.num = calcInput.innerHTML;
          calcInput.innerHTML = '';
        }
  
        window.divide = function() {
          if (multButton.style.background == 'white' || minusButton.style.background == 'white' || plusButton.style.background == 'white') {
            return;
          }
          divideButton.style.background = divideButton.style.background == 'white' ? 'orange' : 'white';
          divideButton.style.color = divideButton.style.color == 'orange' ? 'white' : 'orange';
          window.num = calcInput.innerHTML;
          calcInput.innerHTML = '';
        }
  
        window.minus = function() {
          if (multButton.style.background == 'white' || divideButton.style.background == 'white' || plusButton.style.background == 'white') {
            return;
          }
          minusButton.style.background = minusButton.style.background == 'white' ? 'orange' : 'white';
          minusButton.style.color = minusButton.style.color == 'orange' ? 'white' : 'orange';
          window.num = calcInput.innerHTML;
          calcInput.innerHTML = '';
        }
  
        window.plus = function() {
          if (multButton.style.background == 'white' || minusButton.style.background == 'white' || divideButton.style.background == 'white') {
            return;
          }
          plusButton.style.background = plusButton.style.background == 'white' ? 'orange' : 'white';
          plusButton.style.color = plusButton.style.color == 'orange' ? 'white' : 'orange';
          window.num = calcInput.innerHTML;
          calcInput.innerHTML = '';
        }
        ...
        window.equal = function() {
          ...
          multButton.style.background = divideButton.style.background = minusButton.style.background = plusButton.style.background = 'orange';
          multButton.style.color = divideButton.style.color = minusButton.style.color = plusButton.style.color = 'white';
  
          if (calcInput.innerHTML.includes('.')) {
            calcInput.innerHTML = calcInput.innerHTML.replace('.', ',');
          }
  
          if (calcInput.innerHTML == '-Infinity' || calcInput.innerHTML == 'Infinity' || calcInput.innerHTML == 'NaN') {
    ...
    leftToggle.onclick = () => {
      ...
      weatherUnderMapsDestinations.classList.add('run-appear-maps-destinations-animation');
      searchInput.style.display = mapsDestinations.style.display = weatherUnderMapsDestinations.style.display = 'block';
      setTimeout(() => {
        searchInput.classList.remove('run-appear-search-input-animation');
        mapsDestinations.classList.remove('run-appear-maps-destinations-animation');
        weatherUnderMapsDestinations.classList.remove('run-appear-maps-destinations-animation');
      }, 700);
       }
    }
    ...
    rightToggle.onclick = () => {
      ...
      mapsDestinations.classList.add('run-disappear-maps-destinations-animation');
      weatherUnderMapsDestinations.classList.add('run-disappear-maps-destinations-animation');
      setTimeout(() => {
        searchInput.style.display = mapsDestinations.style.display = weatherUnderMapsDestinations.style.display = 'none';
        searchInput.classList.remove('run-disappear-search-input-animation');
        mapsDestinations.classList.remove('run-disappear-maps-destinations-animation');
        weatherUnderMapsDestinations.classList.remove('run-disappear-maps-destinations-animation');
      }, 200);
      }
    }
  ...
  calendarIcon.onclick = () => {
    ...
    batteryStatus[0].style.boxShadow = 'inset 1px 0px white, inset -1px 0px white, inset 0px 1px white, inset 0px -1px white';
    setTimeout(() => {
  ...
  squareText.onclick = () => {
    ...
    innerCameraBottomContainerTopWrapper.classList.add('run-square-text-animation');
    setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      innerCameraBottomContainerTopWrapper.classList.remove('run-square-text-animation');
    }, 600);
  ...
  videoTextInnerCamera.onclick = () => {
    ...
    innerCameraBottomContainerTopWrapper.classList.add('run-video-text-animation');
    ...
    setTimeout(() => {
      ...
      innerCameraBottomContainerTopWrapper.classList.remove('run-video-text-animation');
    }, 600)
  ...
  photoText.onclick = () => {
    if (innerCameraBottomContainerTopWrapper.style.left == '0px' || innerCameraBottomContainerTopWrapper.style.left == '') {
      return;
    }
  
    innerCameraBottomContainerTopWrapper.classList.add('run-photo-text-animation');
    ...
    setTimeout(() => {
      ...
      innerCameraBottomContainerTopWrapper.classList.remove('run-photo-text-animation');
    }, 600);
  ...
  bottomRemindersContainer.onclick = () => {
    ...
    setTimeout(() => {
      bottomRemindersContainer.classList.remove('run-reminders-appear-animation');
    }, 300);
    isClicked = true;
  }
  ...
  scheduledContainer.onclick = () => {
    if (isClicked) {
      bottomRemindersContainer.classList.add('run-reminders-disappear-animation');
      bottomRemindersContainer.style.top = '345px';
      setTimeout(() => {
        bottomRemindersContainer.classList.remove('run-reminders-disappear-animation');
      }, 300);
      isClicked = false;
    } else {
      return;
  ...
  iPhoneCalculator.onclick = (e) => {
    if (e.target.classList.contains('numbers') || e.target.classList.contains('comma')) {
  ...
  for(let i = 0; i < images.length; i++) {
    images[i].onclick = () => {
      ...
      innerImageTopContainer.style.display = innerImageBottomContainer.style.display = 'block';
  
        let imgStyles = getComputedStyle(images[i]);
  
        openedImage.classList.add('run-appear-animation');
      innerImageBottomContainer.classList.add('run-bottom-container-appear-animation');
      ...
      if (i == 11) {
        imageHeartIcon.src = 'https://image.flaticon.com/icons/svg/1077/1077086.svg';
      } else {
        imageHeartIcon.src = 'https://image.flaticon.com/icons/svg/1077/1077035.svg';
      }
  
        setTimeout(() => {
          openedImage.classList.remove('run-appear-animation');
        innerImageBottomContainer.classList.remove('run-bottom-container-appear-animation');
        }, 150);
        ...
        innerImageBottomContainer.classList.add('run-bottom-container-disappear-animation');
  
        setTimeout(() => {
          openedImage.style.display = innerImageBottomContainer.style.display = 'none';
        }, 100);
  
        setTimeout(() => {
          openedImage.classList.remove('run-disappear-animation');
          innerImageBottomContainer.classList.remove('run-bottom-container-disappear-animation');
        }, 150);
        }
    }
  }
  ...
  innerFavouritesImage.onclick = () => {
    innerFavouritesImageTopContainer.style.display = innerFavouritesImageBottomContainer.style.display = 'block';
  
    let imgStyles = getComputedStyle(innerFavouritesImage);
  
    openedImageInnerFavourites.classList.add('run-appear-animation');
    innerFavouritesImageBottomContainer.classList.add('run-bottom-container-appear-animation');
      ...
      innerFavouritesImageBottomContainer.classList.remove('run-bottom-container-appear-animation');
      }, 150);
    ...
    backArrowInnerFavouriteImage.onclick = () => {
      ...
      innerFavouritesImageBottomContainer.classList.add('run-bottom-container-disappear-animation');
  
      setTimeout(() => {
            openedImageInnerFavourites.style.display = innerFavouritesImageBottomContainer.style.display = 'none';
      }, 100);
  
        setTimeout(() => {
            openedImageInnerFavourites.classList.remove('run-disappear-animation');
        innerFavouritesImageBottomContainer.classList.remove('run-bottom-container-disappear-animation');
        }, 150);
   }
  }
  ...
  homeButton.onclick = () => {
    ...
    if (offContainer.style.display == 'none') {
     ...
     passcodeContainer.classList.add('run-appear-passcode-container-animation');
    }
    ...
    batteryStatus[0].style.boxShadow = 'inset 1px 0px black, inset -1px 0px black, inset 0px 1px black, inset 0px -1px black, inset 1px 1px black, inset -1px -1px black';
    ...
    switch (new Date().getDay()) {
      ...
      break;
      case 3:
      dateInnerStartContainer.innerHTML = 'Wendesday, ' + new Date().getDate();
      break;
      case 4:
